...
for(s in sample_list){
  ...
  for(c in c(1:22)){
    ...
    dat.temp<-subset(dat.temp, select=c(chr, pos_start, gt, coverage))
    ...
    dat.temp<-subset(dat.temp, select=-c(long, gt))
  ...
  #output data
  dat.bed<-subset(dat.bed, select=c(coverage))
  names(dat.bed)<-s
  write.table(dat.bed, paste0( s, ".eh-v5.coverage.txt"), row.names = F, col.names = T, sep="\t", quote=F)
}
