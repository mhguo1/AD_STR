...
for(s in encode_list){
  dat.annot<-read.delim(paste0("/annot/", s, "/hippocampus.", s, ".encode.merged.bed"), header=F, sep="\t", stringsAsFactors = F)
  
    dat.annot$str_id<-paste0(dat.annot$V1, "_", dat.annot$V2, "_", dat.annot$V3)
    dat.annot.se<-makeGRangesFromDataFrame(dat.annot, seqnames = "V1", start.field = "V2", end.field = "V3",keep.extra.columns=TRUE) #Make into GRanges format
    

   #Generate temporary annotation file for histone mark
    dat.annot<-subset(dat.chromhmm, V4==c & V1%in%paste0("chr", seq(1,22)), select=-c(V4))
    c<-gsub("\\/", "_",c)
    dat.annot$str_id<-paste0(dat.annot$V1, "_", dat.annot$V2, "_", dat.annot$V3)
    dat.annot.se<-makeGRangesFromDataFrame(dat.annot, seqnames = "V1", start.field = "V2", end.field = "V3",keep.extra.columns=TRUE) #Make into GRanges format
    ...
    dat.summary$foreground_prop<-dat.summary$foreground_overlap/nrow(dat.foreground) #Proportion of AD STRs overlapping histone mark
    dat.summary$background_prop<-dat.summary$background_overlap/nrow(dat.background) #Proportion of all STRs tested overlapping histone mark
...
write.table(dat.all, "eh_v5.dbscan_eps2.cases_recurrent1.hippocampus_chip_enrichment.txt", row.names = F, col.names = T, sep="\t", quote=F)
